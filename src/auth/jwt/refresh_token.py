...
def refresh_tokens() -> tuple[str, str, datetime.datetime] | None:
    ...
    if not tok:
        view.wrong_message(
            "No stored session. Please login first."
        )
        return None
    ...
    if not user_info:
        view.wrong_message(
            "Missing user info in session. Please login again."
        )
        return None
    ...
    if not refresh_raw or not refresh_exp:
        view.wrong_message(
            "Refresh token unavailable. Please login again."
        )
        return None
    ...
    with Session() as session:
        ...
        if not user or not user.refresh_token_hash:
            view.wrong_message(
                "Cannot validate refresh token. Please login again."
            )
            return None
