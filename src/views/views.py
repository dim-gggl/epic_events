...
class MainView:
    ...
    def display_login(self, access_token, refresh_token, refresh_exp):
        ...
        exp_text = refresh_exp.strftime(
            "%d/%m/%Y"
        ) if hasattr(refresh_exp, "strftime") else str(refresh_exp).split(" ")[0]
        print(
            Panel.fit(
    ...
    def display_logo(self, press_enter: bool = True, centered: bool = True):
        with open("src/views/logo.txt") as file:
            logo_text = file.read()
        logo = Text(logo_text)
    ...
    def display_details(self, obj, fields=None):
        ...
        # Fallback to a generic title when object has no conventional name
        if not name:
